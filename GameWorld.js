...
class GameWorld {
	constructor() {
		...
		this.stick = new Stick(new Vector(this.whiteBall.position.x, this.whiteBall.position.y))
	...
	update() {
		...
		if (!this.whiteBall.moving && this.stick.shot && !this.ballsMoving()) {
			if (this.players[this.turn].totalScore === this.players[this.turn].matchScore) {
	...
	draw() {
		...
		Canvas.drawScore(this.players[this.turn], this.getScore(), this.players[this.turn].color)
	...
	handleBallIn(ball) {
		...
		if (currentPlayer.color === undefined) {
			...
			if (counter > 0) {
				...
				const e8tBallData = {
					totalGames: storedData.totalGames + 1,
					lastWinner: currentPlayer.player,
					player1: {
						username: this.players[0].player,
						winCount:
							storedData.player1.winCount +
							(currentPlayer.player === storedData.player1.username ? 1 : 0)
					},
					player2: {
						username: this.players[1].player,
						winCount:
							storedData.player2.winCount +
							(currentPlayer.player === storedData.player2.username ? 1 : 0)
					}
				}
		...
		if (ball.color !== COLOR.WHITE && ball.color !== COLOR.BLACK) {
			this.balls.splice(this.balls.indexOf(this.balls.find((element) => element === ball)), 1)
		}
	}
